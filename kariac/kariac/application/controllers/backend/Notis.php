...
class Notis extends BE_Controller {
	...
	function __construct() {
		...
		parent::__construct( MODULE_CONTROL, 'noti_module' );
	}
	...
	*/
	function push_message() { 
		...
		if ( $this->input->server( 'REQUEST_METHOD' ) == "POST" ) {
			...
			//$devices = $this->Noti->get_all_by(array('platform_name' => 'android'))->result();
			...
			// $devices = $this->Noti_message->get_all_by(array('os_type' => 'IOS'))->result();
	...
	*/
	function push_message_flutter() { 
		...
		if ( $this->input->server( 'REQUEST_METHOD' ) == "POST" ) {
			...
			$title = htmlspecialchars_decode($this->input->post( 'title' ));
			$message = htmlspecialchars_decode($this->input->post( 'message' ));


			$noti_message = array('title' => $title, 'message' => $message);
