...
import 'package:flutterbuyandsell/ui/common/base/ps_widget_with_appbar_with_two_provider.dart';
...
class _RatingListViewState extends State<RatingListView>
    ...
    _scrollController.addListener(() {
      if (_scrollController.position.pixels ==
        ...
        final RatingListHolder ratingListHolder =
            RatingListHolder(userId: loginUserId);
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
        ...
        child: PsWidgetWithAppBarWithTwoProvider<RatingProvider, UserProvider>(
            appBarTitle: Utils.getString(context, 'rating_list__title') ?? '',
            initProvider1: () {
              ratingProvider = RatingProvider(repo: ratingRepo);
              return ratingProvider;
            },
            onProviderReady1: (RatingProvider provider) async {
              final RatingListHolder ratingListHolder =
                  RatingListHolder(userId: loginUserId);
              await provider.loadRatingList(
                  ratingListHolder.toMap(), loginUserId);
            },
            initProvider2: () {
              userProvider = UserProvider(
                ...
                children: <Widget>[
                  Container(
                    ...
                    child: RefreshIndicator(
                      ...
                      onRefresh: () {
                        final RatingListHolder ratingListHolder =
                            RatingListHolder(userId: widget.itemUserId);
