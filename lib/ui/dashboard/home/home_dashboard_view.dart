...
class _RecentProductHorizontalListWidget extends StatefulWidget {
  ...
  final Animation<double> animation;
  final PsValueHolder psValueHolder;
  final dynamic categoryProvider;
...
Widget howtoWidgetItem(
    String title,
    String leadingIconPath,
    String trailingIconPath,
    Color firstColor,
    Color secondColor,
    dynamic psValueHolder,
    BuildContext context) {
  return GestureDetector(
    onTap: () async {
      if (await Utils.checkInternetConnectivity()) {
        String userId = Utils.checkUserLoginId(psValueHolder);
        if (userId == "") {
          Navigator.pushNamed(
            context,
    ...
    child: Card(
      child: Container(
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              Image.asset(leadingIconPath, width: 30, height: 30),
              SizedBox(width: 10),
              Container(
                width: 100,
                child: Text(
                  ...
                  softWrap: true,
                  style: TextStyle(
                      color: title == "How to donate to charity"
                          ? Colors.black
                          : Colors.white),
                ),
              ),
