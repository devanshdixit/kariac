      ...
      children: <Widget>[
        Column(
          ...
          children: <Widget>[
                ...
                await showDialog<dynamic>(
                    ...
                    builder: (BuildContext context) {
                      return RatingInputDialog(
                        psValueHolder: widget.psValueHolder,
                        itemUserId: widget.itemDetail.user.userId,
                        buyerUserId: widget.buyerUserId,
                        sellerUserId: widget.sellerUserId,
                        itemDetail: widget.itemDetail,
                      );
            ...
            // else
            //   // (widget.sellerUserId == widget.psValueHolder.loginUserId)
            //   InkWell(
            //     onTap: () async {
            //       Utils.psPrint('Buyer user id : ' + widget.buyerUserId);
            //       Utils.psPrint('Seller user id : ' + widget.sellerUserId);
    ...
    if (images.isNotEmpty) {
      ...
      if (widget.chatFlag == PsConst.CHAT_FROM_BUYER) {
        _apiStatus = await provider.postChatImageUpload(
          ...
          widget.itemData.id,
          PsConst.CHAT_TO_BUYER,
          await Utils.getImageFileFromAssets(images[0]),
        );
      } else {
        _apiStatus = await provider.postChatImageUpload(
          widget.sellerUserId,
...
class _ItemInfoWidgetState extends State<ItemInfoWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      decoration: BoxDecoration(
          borderRadius: BorderRadius.circular(PsDimens.space4),
          color: Utils.isLightMode(context) ? Colors.white70 : Colors.black12),
      padding: const EdgeInsets.all(PsDimens.space12),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          ...
          Consumer<GetChatHistoryProvider>(builder: (BuildContext context,
            ...
            return InkWell(
                ...
                child: widget.chatFlag != PsConst.CHAT_FROM_BUYER && _showOffer
                    ? Container(
                        ...
                        child: Ink(
                          ...
                          child: Padding(
                            ...
                            child: Text(
                              Utils.getString(
                                  context, 'chat_view__make_offer_button_name'),
                              style: Theme.of(context)
                                  .textTheme
                                  .bodyText2
                                  .copyWith(color: Colors.white),
